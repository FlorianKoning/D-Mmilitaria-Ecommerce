          ...
          <ul role="list" class="divide-y divide-gray-200 border-b border-t border-gray-200">
            @if ($cart != null)
                @foreach ($cart as $item)
                    @php
                        $prices = $cartService::price($item->id);
                    @endphp
                            ...
                            <div class="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
                                <div>
                                    ...
                                    @if (count($prices) > 1)
                                        <div class="flex flex-row gap-x-1">
                                            <span class="line-through text-sm text-red-500">{{ $prices['oldPrice'] }}</span>
                                            <p class="text-sm text-gray-900">{{ $prices['newPrice'] }}</p>
                                        </div>
                                    @else
                                        <div class="flex flex-row gap-x-1">
                                            <p class="text-sm text-gray-900">{{ $prices['oldPrice'] }}</p>
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="mt-4 sm:mt-0 sm:pr-9">
                                    <div class="grid w-full max-w-16 grid-cols-1">
                                        <select name="quantity-0" aria-label="Quantity, Basic Tee" class="col-start-1 row-start-1 appearance-none rounded-md bg-white py-1.5 pl-3 pr-8 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-gray-300 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-navBackground sm:text-sm/6">
                                            <option {{ $item->amount == '1' ? 'selected' : '' }} value="1">1</option>
                                            <option {{ $item->amount == '2' ? 'selected' : '' }} value="2">2</option>
                                            <option {{ $item->amount == '3' ? 'selected' : '' }} value="3">3</option>
                                            <option {{ $item->amount == '4' ? 'selected' : '' }} value="4">4</option>
                                            <option {{ $item->amount == '5' ? 'selected' : '' }} value="5">5</option>
                                            <option {{ $item->amount == '6' ? 'selected' : '' }} value="6">6</option>
                                            <option {{ $item->amount == '7' ? 'selected' : '' }} value="7">7</option>
                                            <option {{ $item->amount == '8' ? 'selected' : '' }} value="8">8</option>
                                        </select>
                                    </div>
